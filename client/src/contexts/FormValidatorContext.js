...
export const FormValidatorProvider = ({
    ...
    const GameValidator = (e) => {
        ...
        if(target === "name" && value.length < 2){
            formErrors.name = 'The name of the game must be at least 2 characters long'
        };
        ...
        if(target === "platform" && value.length < 1){
        ...
        if(target === "description" && value.length < 10){
            formErrors.description = 'The description must be at least 10 characters long'
    };
